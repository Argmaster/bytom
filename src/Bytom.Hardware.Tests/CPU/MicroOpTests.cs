...
namespace Bytom.Hardware.CPU.Tests
{
    public class InstructionDecodeTests
    {
        ...
        public void TestDecodeNop()
        {
            ...
            Assert.That(core!.pipeline.Count, Is.EqualTo(offset));
        }
        ...
        private int checkUpdateInstructionPointer(int offset)
        {
            ...
            Assert.That(core.pipeline[offset], Is.TypeOf<ALUOperation32>());
            var alu_operation = (ALUOperation32)core.pipeline[offset];
        ...
        private int checkContinueExecution(int offset)
        {
            ...
            Assert.That(core.pipeline[offset], Is.TypeOf<InstructionDecode>());
            var instruction_decode_next = (InstructionDecode)core.pipeline[offset];
        ...
        {
            Assert.That(core!.pipeline[offset], Is.TypeOf<CopyRegister>());
            var copy_register = (CopyRegister)core.pipeline[offset];
        ...
        {
            Assert.That(core!.pipeline[offset], Is.TypeOf<WriteMemory>());
            var write_memory = (WriteMemory)core.pipeline[offset];
